...
export function ExpenseSummary({ data }: ExpenseSummaryProps) {
  ...
  // Calculate totals based on wallet type
  const bankVND = data.filter(item => item.wallet_type === 'Ngân Hàng')
    .reduce((sum, item) => sum + item.amount_vnd, 0);

  const bankUSD = data.filter(item => item.wallet_type === 'Ngân Hàng')
    .reduce((sum, item) => sum + item.amount_usd, 0);

  const binanceUSDT = data.filter(item => item.wallet_type === 'Binance')
    .reduce((sum, item) => sum + item.amount_usdt, 0);

  const upworkUSD = data.filter(item => item.wallet_type === 'Upwork')
    .reduce((sum, item) => sum + item.amount_usd, 0);

  const cashVND = data.filter(item => item.wallet_type === 'Tiền Mặt')
    .reduce((sum, item) => sum + item.amount_vnd, 0);
