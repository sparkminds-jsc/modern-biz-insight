...
export interface RevenueInvoiceFile {
  name: string;
  path?: string;
  url?: string;
  size?: number;
...
export function RevenueInvoiceFilesDialog({ open, onClose, revenue, onSaved }: Props) {
  const [files, setFiles] = useState<RevenueInvoiceFile[]>([]);
  const [uploading, setUploading] = useState(false);

  useEffect(() => {
    if (open) {
      setFiles(Array.isArray(revenue?.invoice_files) ? (revenue.invoice_files as RevenueInvoiceFile[]) : []);
    }
  }, [open, revenue]);

  const persist = async (next: RevenueInvoiceFile[]) => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const uploaded: RevenueInvoiceFile[] = [];
      for (const file of selected) {
        const safeName = file.name.replace(/[^\w.\-]+/g, '_');
        const path = `revenue/${revenue.id}/${Date.now()}_${safeName}`;
  ...
  const handleDelete = async (file: RevenueInvoiceFile) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Hóa đơn doanh thu</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <div>
            <input
              id="revenue-invoice-upload"
              type="file"
              multiple
              className="hidden"
            ...
            <Button asChild disabled={uploading} variant="outline">
              <label htmlFor="revenue-invoice-upload" className="cursor-pointer">
